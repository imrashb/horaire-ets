...
public class CoursParser {
    ...
    private static Pattern activitePattern = Pattern.compile("^([a-zA-Z]{3})\\W(\\d{2}):(\\d{2})\\W-\\W(\\d{2}):(\\d{2})\\W(.*)\\W.*(D|P|H|C)");
    ...
    private Cours getCoursFromLine(String line) {
        ...
        if(match.find()) {
            ...
            for(Cours cours : listeCours) {
                ...
                if(cours.getId().equalsIgnoreCase(id)) {
                    return cours;
                }
            }
    ...
    private Groupe getGroupeFromLine(String line) {
        ...
        Groupe groupe = getGroupe(line);

        if(groupe != null) return groupe;

        Activite activite = getActivite(line);
        if(activite != null) {
    ...
    private Groupe getGroupe(String line) {
    ...
    private Activite getActivite(String line) {
        Matcher match = activitePattern.matcher(line);
        ...
        if(match.find()) {
            ...
            Activite activite = new Activite(match.group(6), match.group(7), sch);
            return activite;
        }
        return null;
    }
